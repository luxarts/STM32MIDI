...
__ALIGN_BEGIN uint8_t USBD_CDC_CfgFSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END = {
	...
	0x09, 0x02, 0x65 + 6 + 9 + 1 + 1, 0x00, 0x02, 0x01, 0x00, 0x80, 0x0A,

	// The Audio Interface Collection
	0x09, 0x04, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00, // Standard AC Interface Descriptor
	0x09, 0x24, 0x01, 0x00, 0x01, 0x09, 0x00, 0x01, 0x01, // Class-specific AC Interface Descriptor
	0x09, 0x04, 0x01, 0x00, 0x02, 0x01, 0x03, 0x00, 0x00, // MIDIStreaming Interface Descriptors
	0x07, 0x24, 0x01, 0x00, 0x01, 0x25 + 6 + 9, 0x00,   // Class-Specific MS Interface Header Descriptor
	...
	0x09, 0x05, CDC_IN_EP, 0x02, 0x40, 0x00, 0x00, 0x00, 0x00, 0x06,
	0x25, 0x01, 0x02, 0x03, 0x13,
};
/**
 * @}
 */
...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
	...
	if (pdev->pClassData == NULL) {
		...
		USBD_LL_PrepareReceive(pdev, CDC_OUT_EP, hcdc->RxBuffer, CDC_DATA_OUT_PACKET_SIZE);
	}
	return ret;
}
		...
		if (req->wLength) {
			if (req->bmRequest & 0x80U) {
				((USBD_CDC_ItfTypeDef*) pdev->pUserData)->Control(req->bRequest,
						(uint8_t*) (void*) hcdc->data, req->wLength);
